...
import InputContainer from './VInputContainer';
...
export default class VForm extends InputContainer {
	_isMounted = false;
	...
	static propTypes = {
		...
		errorMessage: PropTypes.oneOfType([PropTypes.object, PropTypes.string, PropTypes.node]),
	...
	handleSubmit = async (e) => {
		...
		if (!this.state.submitted && this._isMounted) {
	...
	render() {
		...
		return <Tag noValidate action="#" {...attributes} className={classes} onSubmit={this.handleSubmit} />;
	}
	...
	updateInputs() {
		...
		this.throttledUpdateInputs = _throttle(() => {
			Object.keys(this._updaters).forEach(
				(inputName) => this._updaters[inputName] && this._inputs[inputName] && this._updaters[inputName].call(this._inputs[inputName], {})
			);
		}, 250);
		this.updateInputs();
	}
					...
					const promise = new Promise((resolve, reject) => {
						...
						if (typeof ruleProp[rule] === 'function') {
							...
							if (ruleProp[rule].enabled === false) {
								...
								ruleResult = Validator[rule](val, context, ruleProp[rule], input, callback);
							}
						}
