...
const radioPropTypes = { ...VInput.propTypes, ...{ customInput: PropTypes.bool } };
delete radioPropTypes.name;

export default class VRadio extends Component {
	static contextTypes = {
		...VInput.contextTypes,
		...{
		...
		return (
			<FormGroup check inline={groupProps.inline} disabled={attributes.disabled || attributes.readOnly}>
				<Input
					...
					required={groupProps.required}
				/>
				<Label check for={id || `radio-${groupProps.name}-${this.props.value}`}>
					{this.props.label}
				</Label>
			</FormGroup>
		);
