...
export default class VCheckboxGroup extends Component {
	static propTypes = {
		...FormGroup.propTypes,
		...{
	...
	getChildContext() {
		...
		const updateGroup = async (e, value) => {
			...
			if (!this.context.FormCtrl.isDirty(this.props.name)) {
	...
	updateInputs() {
		this._inputs.forEach((input) => findDOMNode(input).firstChild.setCustomValidity('Invalid.') && input.setState.call(input, {}));
	...
	render() {
		...
		return (
			<FormGroup tag="fieldset" {...attributes} className={groupClass}>
				...
				<VFeedback>{validation.errorMessage}</VFeedback>
			</FormGroup>
		);
	}
}
