...
const checkPropTypes = { ...VInput.propTypes, ...{ customInput: PropTypes.bool } };
delete checkPropTypes.name;

export default class VCheckbox extends Component {
	static contextTypes = {
		...VInput.contextTypes,
		...{
	...
	render() {
		const { className, id, customInput, ...attributes } = this.props;
		...
		return (
			<FormGroup check inline={groupProps.inline} disabled={attributes.disabled || attributes.readOnly}>
				<Input
					...
					required={groupProps.required}
				/>
				<Label check for={id || `checkbox-${groupProps.name}-${this.props.value}`}>
					{this.props.label}
				</Label>
			</FormGroup>
		);
